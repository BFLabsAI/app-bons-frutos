...
// If the user doesn't have a license, we might need a fallback or basic animation.
// Since I cannot register premium plugins without the file, I will wrap registration.
try {
    gsap.registerPlugin(useGSAP);
    // gsap.registerPlugin(SplitText); // Commented out to avoid errors if not available, will use standard GSAP
} catch (e) {
    console.warn("GSAP plugins registration failed", e);
}
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Tint with Brand Color (Olive #606A30)
    vec3 brandColor = vec3(0.37, 0.41, 0.18);
    vec3 tinted = mix(color.rgb, brandColor, 0.65);
...
interface HeroProps {
    ...
    badgeText?: string;
    badgeLabel?: string;
    ctaButtons?: Array<{ text: string; href?: string; action?: () => void; primary?: boolean }>;
    microDetails?: Array<string>;
    children?: React.ReactNode; // Added children to allow passing the login form
}
...
export default function Hero({
    ...
    useGSAP(
        () => {
            ...
            // Cleaned up SplitText usage as it requires premium. 
            // Using standard fadeIn for now to ensure it works.
            ...
            tl.to(headerRef.current, { autoAlpha: 1, y: 0, filter: 'blur(0px)', duration: 0.8 }, 0.2);
            ...
            if (childrenRef.current) {
                gsap.set(childrenRef.current, { autoAlpha: 0, scale: 0.95 });
                tl.to(childrenRef.current, { autoAlpha: 1, scale: 1, duration: 0.6 }, '-=0.3');
            }

            if (microRef.current) {
                gsap.set(microRef.current, { autoAlpha: 0 });
                tl.to(microRef.current, { autoAlpha: 1, duration: 0.5 }, '-=0.2');
            }
        },
        { scope: sectionRef },
    );
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden flex items-center justify-center lg:justify-start">
            <ShaderBackground />

            <div className="relative mx-auto flex w-full max-w-7xl items-center gap-12 px-6 sm:gap-16 sm:px-10 lg:px-16 flex-col lg:flex-row">

                {/* Left Content */}
                <div className="flex flex-col items-start gap-6 pt-12 lg:pt-0 max-w-2xl">
                    ...
                    <h1 ref={headerRef} className="text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl">
                        {title}
                    </h1>

                    <p ref={paraRef} className="text-left text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg">
                        {description}
                    </p>
                ...
                <div ref={childrenRef} className="w-full max-w-md lg:ml-auto">
                    {children}
                </div>
